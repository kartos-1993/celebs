...
export function TableSkeleton({ rows = 6, columns = 5, className }: TableSkeletonProps) {
  return (
    ...
    >
      <div className="flex h-9 animate-pulse items-center gap-4 border-b bg-muted/50 px-3">
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div
          key={rowIndex}
          className="flex animate-pulse items-center gap-4 border-b px-3 py-3 last:border-0"
