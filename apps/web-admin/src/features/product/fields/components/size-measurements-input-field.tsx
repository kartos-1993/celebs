...
export function SizeMeasurementsInputField({ field }: UiProps) {
  ...
  React.useEffect(() => {
    ...
    const newSizesState: SizeEntry[] = selectedSizes.map((sizeName) => {
      ...
      if (existing) {
        const syncCols = (list: MeasurementItem[] | undefined, targetCols: string[]): MeasurementItem[] => {
            ...
            return (
              <TableRow key={sizeName}>
                ...
                {chart.columns.map((c) => {
                  ...
                  const sizesErrors = formState.errors.sizes as Record<string, Record<string, Array<{ value?: { message?: string } }>>> | undefined;
                  const cellError =
                    colIndex !== -1
  ...
  return (
    <div className="col-span-full space-y-4 rounded-xl border bg-card p-4 shadow-2xs">
      ...
      {/* Tabs for Multiple Charts if category has product vs body measurements */}
      {charts.length > 1 ? (
