...
export const ProductFormActionsContainer = memo(
    ({ sections, schemaReady, isDirty, isSubmitting, onSaveAsDraft, onCancel }: ProductFormActionsContainerProps) => {
        const isReady = schemaReady && sections.length > 0 && sections.every((section) => section.status);
        return (
            <ProductFormActions
                isDirty={isDirty}
                isReady={isReady}
