...
describe('Product Specifications & Size Measurements', () => {
  const baseValidProduct = {
    ...
    brand: "Manfinity",
    description: "High-quality knit polo shirt with ribbed design, long sleeves, and classic lapel collar.",
    price: 1200,
  ...
  it('should fail validation if discountedPrice is higher than price', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('Discounted price must be less than the regular price');
    }
  });
});
