...
const Orders: React.FC = () => {
  ...
  const [newStatus, setNewStatus] = useState<'PACKED' | 'HANDED_OVER' | 'DELIVERED' | 'CANCELLED'>('PACKED');
  ...
  const handleOpenFulfillModal = (item: OrderItemUI) => {
    ...
    setNewStatus(item.itemStatus === 'PENDING' ? 'PACKED' : item.itemStatus === 'PACKED' ? 'HANDED_OVER' : 'DELIVERED');
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PENDING':
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-amber-100 text-amber-800 border border-amber-300">📥 Needs Packing</span>;
      case 'PACKED':
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 border border-blue-300">📦 Packed & Ready</span>;
      case 'HANDED_OVER':
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-purple-100 text-purple-800 border border-purple-300">🚚 In Transit</span>;
      case 'DELIVERED':
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300">✅ Delivered</span>;
      case 'CANCELLED':
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Vendor Order Fulfillment</h1>
          <p className="text-sm text-gray-500">Pick, pack, attach tracking codes, and dispatch incoming orders across Nepal.</p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50/50">
              ...
              <TableHead className="text-xs font-semibold text-gray-700">Customer & Address</TableHead>
              <TableHead className="text-xs font-semibold text-gray-700">Product & Variant</TableHead>
              <TableHead className="text-xs font-semibold text-gray-700">Amount & Payment</TableHead>
                ...
                <TableRow key={ord.id} className="hover:bg-gray-50/60 transition-colors">
                  ...
                  <TableCell>
                    ...
                    <div className="text-[11px] text-gray-500">
                      Color: <span className="font-medium text-gray-700">{ord.colorVariantName}</span> | Size: <span className="font-medium text-gray-700">{ord.size}</span> | Qty: {ord.quantity}
                    </div>
                  </TableCell>

                  <TableCell>
                    <div className="text-xs font-semibold text-slate-900">NPR {ord.totalAmount.toLocaleString()}</div>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[10px] font-mono px-1.5 py-0.5 bg-gray-100 rounded text-gray-700 font-semibold">{ord.paymentMethod}</span>
                      <span className={`text-[10px] font-medium ${ord.paymentStatus === 'COMPLETED' ? 'text-emerald-600' : 'text-amber-600'}`}>
                        {ord.paymentStatus}
                      </span>
                    </div>
                  </TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-2 text-xs">
              ...
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-1">
                ...
                <div className="text-slate-600">
                  Variant: <span className="font-medium text-slate-900">{selectedItem.colorVariantName}</span> | Size: <span className="font-medium text-slate-900">{selectedItem.size}</span> | Qty: {selectedItem.quantity}
                </div>
                <div className="text-slate-500 text-[11px]">
                  Shipping to: {selectedItem.customerName} ({selectedItem.cityArea}, {selectedItem.provinceDistrict})
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={newStatus} onValueChange={(val: any) => setNewStatus(val)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="CANCELLED">❌ CANCELLED (Out of stock / Cancelled)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={courier} onValueChange={setCourier}>
                  ...
                  <SelectContent>
                    <SelectItem value="Upaya Logistics">Upaya Logistics (Kathmandu Valley & Express)</SelectItem>
                    <SelectItem value="Nepal Can Move">Nepal Can Move (Nationwide Nepal Delivery)</SelectItem>
          ...
          <DialogFooter className="flex justify-end gap-2">
            ...
            <Button size="sm" onClick={handleSaveFulfillment} className="bg-indigo-600 hover:bg-indigo-700 text-white">
