...
async function mkStaff(storeId: string, permissions: string[] = []): Promise<ActorHandle> {
  ...
  return { userId: user.id, email: user.email, token: signJwtToken({ userId: user.id, sessionId: session.id }) };
}
...
describe('Access Control Matrix', () => {
  it(
    'enforces identity → context → lifecycle → permission across every route class',
    async () => {
      // ── fixture universe ──
