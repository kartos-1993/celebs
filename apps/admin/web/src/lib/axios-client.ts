...
const PRODUCT_API_URL = import.meta.env.VITE_PRODUCT_API_URL || FALLBACK_PRODUCT_API_URL;
...
export const ProductAPI = axios.create({ ...baseOptions, baseURL: PRODUCT_API_URL });

// Refresh client (using Auth API for refresh token)
export const APIRefresh = axios.create({ ...baseOptions, baseURL: AUTH_API_URL });
  ...
  return async (error: AxiosError) => {
    ...
    if (status === 401 && data === "Unauthorized") {
      try {
        ...
        // Safe check for config existence and type
        if (error.config) {
          return instance(error.config as InternalAxiosRequestConfig); 
        }
        return Promise.reject({ message: "Failed to retry request" });
      } catch (refreshError) {
