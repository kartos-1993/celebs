...
license: MIT
metadata:
  tags: 'YAGNI, Simplicity, Anti-Overengineering, Minimalist'
...
4. **Native platform feature covers it?** Standard HTML over custom controls, CSS over JS, DB constraints/indices over app code, standard HTTP status codes over custom envelopes.
...
- **Complex request?** Ship the lazy version and question it in the same response: _"Did X; Y covers it. Need full X? Say so."_ Never stall on an answer you can default.
- **Two stdlib options, same size?** Take the one that's correct on edge cases. Lazy means writing less code, not picking the flimsier algorithm.
- **Zero defensive fallback cascades:** No `data?.data?.data ?? data?.data ?? []`. If the API or contract doesn't match, fix the contract at the source; do not hide contract violations behind multi-tier fallbacks.
- **Mark deliberate simplifications** that cut a real corner with a known ceiling (e.g. global lock, naive heuristic) with a `// ponytail:` comment naming the ceiling and upgrade path (`// ponytail: table lock, per-row locks if throughput matters`).
...
- **Lazy code without its check is unfinished.** Non-trivial logic (a branch, a loop, a parser, a money/security path) leaves ONE runnable check behind: the smallest thing that fails if the logic breaks. Run localized unit tests or test commands. Trivial one-liners need no test; YAGNI applies to tests too.

## Boundaries

Ponytail governs what you build, not how you talk. "stop ponytail" / "normal mode": revert. Level persists until changed or session end. The shortest path to done is the right path.
